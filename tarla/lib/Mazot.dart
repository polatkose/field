...
class Mazot extends StatefulWidget {
  const Mazot({super.key});

  @override
  State<Mazot> createState() => _MazotState();
}

class _MazotState extends State<Mazot> {
  ...
  Future<List<PetrolFiyatlari>> petrolListesi(int sehirId) async {
    var petrolListesi = await Tarimdao().petrolList(sehirId);
    return petrolListesi;
  }
          ...
          child: Column(
            children: [
              ...
              SizedBox(
                ...
                height: 600,
                child: FutureBuilder<List<PetrolFiyatlari>>(
                  future: petrolListesi(selectedCityId),
                  builder: (context, snapshot) {
                    if (snapshot.connectionState == ConnectionState.waiting) {
                      ...
                      var petrolListesi = snapshot.data!;
                      return ListView.builder(
                        physics: BouncingScrollPhysics(),
                        itemCount: petrolListesi.length,
                        itemBuilder: (context, index) {
                          var ofis = petrolListesi[index];
                          return Container(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      Container(
                                        ...
                                        child: Image.asset("resimler/${ofis.Ofis_Resim}"),
                                      ),
                                      Text("${ofis.Ofis_Adi}",style: TextStyle(fontSize: 15, color: Colors.grey),),
                                      Text("${ofis.Ofis_Fiyat}₺", style: TextStyle(fontWeight: FontWeight.bold),),
                                    ],
                                  ),
                                ),
                                Container(
